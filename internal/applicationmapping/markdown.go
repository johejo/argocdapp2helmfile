...
func Markdown() []byte {
	...
	output.WriteString(`# Application mapping reference
...
`)
	for _, entry := range entries {
		fmt.Fprintf(
			&output,
			"| %s | %s | %s |\n",
			markdownCell(entry.Input),
			markdownCell(entry.Output),
			markdownCell(entry.Notes),
		)
	}
...
The required fields are ` + "`metadata.name`" + `, the manifest source's ` +
		"`repoURL`" + `, and either ` + "`chart`" + ` plus
` + "`targetRevision`" + `
